...
export default function CatProfilePage({
  ...
  const resolveEditable = value => {
    if (Array.isArray(value)) return value.map(resolveEditable);
    if (value && typeof value === 'object') {
      if (Object.hasOwn(value, 'pl') || Object.hasOwn(value, 'ru') || Object.hasOwn(value, 'en')) return text(value);
      return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, resolveEditable(item)]));
    }
    return value;
  };
  ...
  return <div className="min-h-screen bg-[#f7f8f4] text-slate-950 selection:bg-sky-200">
    ...
    <main>
      ...
      <section className="mx-auto grid max-w-6xl gap-5 px-5 pb-20 sm:px-8 lg:grid-cols-2"><ListCard title={ui.likes} intro={cat.likesIntro} items={cat.likes} tone="amber" /><ListCard title={ui.respect} intro={cat.boundariesIntro} items={cat.boundaries} tone="slate" /></section>
      <section className="mx-auto max-w-6xl px-5 pb-20 sm:px-8"><div className="grid gap-10 rounded-[3rem] border border-slate-200 bg-white p-7 sm:p-10 lg:grid-cols-[.8fr_1.2fr]"><div><p className="text-sm font-extrabold uppercase tracking-[.18em] text-sky-600">{ui.day}</p><h2 className="mt-3 text-4xl font-black tracking-tight">{cat.routineTitle}</h2><p className="mt-4 leading-relaxed text-slate-600">{cat.routineIntro}</p></div><div className="space-y-4">{cat.routine.map(item => <Routine key={item.time} time={item.time} text={item.text} />)}</div></div></section>
