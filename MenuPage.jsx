...
const copy = {
  pl: { back: 'Wróć do kawiarni', label: 'Jedzenie i napoje', title: 'Kocia karta.', intro: 'Proste składniki, sezonowe smaki i kawa, dla której warto zostać chwilę dłużej.', vegan: 'opcje vegan', allergens: 'alergeny u obsługi', fee: 'Opłata opiekuńcza: 8 zł / osoba', feeText: 'Cała kwota wspiera karmę, leczenie i przygotowanie kotów do adopcji.', note: 'Menu jest częścią projektu koncepcyjnego. Dostępność dań może zmieniać się sezonowo. Informację o składnikach i alergenach zawsze potwierdź u obsługi.' },
  ru: { back: 'Вернуться в кафе', label: 'Еда и напитки', title: 'Кошачье меню.', intro: 'Простые ингредиенты, сезонные вкусы и кофе, ради которого стоит задержаться.', vegan: 'веганские опции', allergens: 'аллергены уточняйте у команды', fee: 'Забота о котах: 8 zł / человек', feeText: 'Вся сумма помогает оплачивать корм, лечение и подготовку котов к усыновлению.', note: 'Меню — часть концептуального проекта. Состав блюд может меняться по сезону. Всегда уточняйте ингредиенты и аллергены у команды.' },
  en: { back: 'Back to the café', label: 'Food and drinks', title: 'The cat menu.', intro: 'Simple ingredients, seasonal flavours and coffee worth staying for a little longer.', vegan: 'vegan options', allergens: 'ask the team about allergens', fee: 'Care fee: 8 zł / person', feeText: 'Every złoty supports food, treatment and preparing cats for adoption.', note: 'This menu is part of a concept project. Availability changes seasonally. Please confirm ingredients and allergens with the team.' },
};
const groupCopy = {
...
export default function MenuPage() {
  ...
  return <div className="min-h-screen bg-[#f7f8f4] text-slate-950">
    <nav className="sticky top-0 z-40 border-b border-white/60 bg-[#f7f8f4]/90 backdrop-blur-xl"><div className="mx-auto flex h-20 max-w-6xl items-center justify-between gap-3 px-5 sm:px-8"><a href="/" className="flex items-center gap-2.5"><span className="grid h-9 w-9 place-items-center rounded-full bg-sky-500 text-white"><Cat size={19}/></span><span className="text-lg font-extrabold tracking-tight">Niebieski Kot<span className="text-sky-500">.</span></span></a><div className="flex items-center gap-2"><LanguageSelect/><a href="/" className="flex items-center gap-2 rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm font-bold transition hover:border-sky-400 hover:text-sky-600"><ArrowLeft size={16}/> {t.back}</a></div></div></nav>
