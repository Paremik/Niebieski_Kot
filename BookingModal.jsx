...
export default function BookingModal({
  ...
  useEffect(() => { if (open) {setForm(initialForm); setSent(false); setError(false);} }, [open]);
  if (!open) return null;
  ...
  const submit = event => {
    ...
    if (!form.name.trim() || !form.consent || !getBookingTimes(form.date, new Date(), bookingEvent?.index).includes(form.time)) {setError(true); return;}
    setSent(true);
  };
  ...
  return <div className="fixed inset-0 z-[70] grid place-items-center overflow-y-auto bg-slate-950/65 p-4 backdrop-blur-sm" onMouseDown={event => event.target === event.currentTarget && close()}>
    <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="booking-title" className="relative my-auto w-full max-w-2xl overflow-hidden rounded-[2.25rem] bg-[#f7f8f4] shadow-2xl">
      ...
      {sent ? <div className="px-7 pb-12 pt-24 text-center sm:px-12"><span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-emerald-100 text-emerald-600"><Check size={30} /></span><p className="mt-6 text-sm font-extrabold uppercase tracking-[.18em] text-sky-600">{tr("Formularz demonstracyjny")}</p><h2 id="booking-title" tabIndex={-1} className="mt-3 text-4xl font-black tracking-tight">{tr("Termin zapisany w podglądzie.")}</h2><p className="mx-auto mt-4 max-w-lg leading-relaxed text-slate-600">{tr("To koncepcyjna wersja strony, dlatego dane nie zostały nigdzie wysłane. Po uruchomieniu kawiarni podłączymy prawdziwe potwierdzenia e-mail.")}</p><button type="button" onClick={close} className="mt-8 rounded-full bg-slate-950 px-7 py-3.5 font-bold text-white transition hover:bg-sky-600">{tr("Wróć na stronę")}</button></div> : <>
        <header className="bg-sky-500 px-7 pb-8 pt-20 text-white sm:px-10"><p className="text-sm font-extrabold uppercase tracking-[.18em] text-sky-100">{tr("Spokojne 90 minut")}</p><h2 id="booking-title" tabIndex={-1} className="mt-2 text-4xl font-black tracking-tight">{tr("Zarezerwuj stolik.")}</h2><p className="mt-3 max-w-lg text-sm leading-relaxed text-sky-50">{tr("Wybierz termin i zostaw kontakt. Na tym etapie formularz działa jako bezpieczna demonstracja.")}</p></header>
        <form onSubmit={submit} className="grid gap-5 p-7 sm:grid-cols-2 sm:p-10">
          ...
          {error && <p role="alert" className="text-sm text-rose-700 sm:col-span-2">{tr('Wybierz dostępny termin w przyszłości. W poniedziałki jesteśmy zamknięci.')}</p>}
          ...
          <label className="flex items-start gap-3 text-sm leading-relaxed text-slate-600 sm:col-span-2"><input required type="checkbox" checked={form.consent} onChange={update('consent')} className="mt-1 h-4 w-4 accent-sky-500" /><span>{tr("Rozumiem, że to formularz demonstracyjny i dane nie zostaną wysłane ani zapisane.")}</span></label>
          <button type="submit" className="rounded-full bg-slate-950 px-7 py-3.5 font-extrabold text-white transition hover:-translate-y-0.5 hover:bg-sky-600 sm:col-span-2">{tr("Pokaż potwierdzenie")}</button>
        </form>
      </>}
    </section>
  </div>;
