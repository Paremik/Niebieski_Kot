...
export const defaultData = {
  ...
  cats: [
    { id:'luna', slug:'luna', name:'Luna', status:'resident', linkLabel:localized('Poznaj Lunę','Познакомиться с Луной','Meet Luna'), note:localized('4 lata · spokojna obserwatorka','4 года · спокойная наблюдательница','4 years · a calm observer'), intro:localized('Najchętniej siedzi przy oknie i sama wybiera moment na głaskanie.','Любит сидеть у окна и сама выбирает момент для ласки.','She loves sitting by the window and chooses when it is time for affection.'), profile:defaultCatProfile('luna'), enabled:true },
    { id:'mochi', slug:'mochi', name:'Mochi', status:'resident', linkLabel:localized('Poznaj Mochiego','Познакомиться с Мочи','Meet Mochi'), note:localized('6 lat · mistrz drzemek','6 лет · мастер сна','6 years · master napper'), intro:localized('Kocha miękkie koce, spokojne rozmowy i ludzi z książką na kolanach.','Любит мягкие пледы, спокойные разговоры и людей с книгой на коленях.','He loves soft blankets, quiet conversation and people reading a book.'), profile:defaultCatProfile('mochi'), enabled:true },
    { id:'pixel', slug:'pixel', name:'Pixel', status:'adoption', linkLabel:localized('Poznaj Pixela','Познакомиться с Пикселем','Meet Pixel'), note:localized('2 lata · pierwszy do zabawy','2 года · всегда первый в игре','2 years · always first to play'), intro:localized('Wędkę wypatrzy z drugiego końca sali, a potem zasypia pod stolikiem.','Удочку заметит с другого конца зала, а потом уснёт под столиком.','He spots a teaser toy across the room, then falls asleep under a table.'), profile:defaultCatProfile('pixel'), enabled:true }
  ],
  revision: 0,
  updatedAt: null
};
...
export function normalizeAdminData(input) {
  ...
  result.events = (Array.isArray(source.events) ? source.events : defaultData.events).slice(0,30).map((row,i)=>{const fallback=defaultData.events[i%defaultData.events.length];return {id:stableId(row?.id,`event-${i}`),tag:localText(row?.tag,fallback.tag),title:localText(row?.title,fallback.title),description:localText(row?.description,fallback.description,2000),date:localText(row?.date,fallback.date),places:trim(row?.places??'0',4),status:['open','planned','closed'].includes(row?.status)?row.status:'planned',enabled:row?.enabled!==false};});
  ...
  result.cats=(Array.isArray(source.cats)?source.cats:defaultData.cats).slice(0,20).map((row,i)=>{const fallback=defaultData.cats.find(cat=>cat.slug===row?.slug)||defaultData.cats[i]||defaultData.cats[0];return {id:stableId(row?.id,`cat-${i}`),slug:stableId(row?.slug,fallback.slug??`cat-${i}`),name:trim(row?.name??fallback.name,80),status:['resident','adoption','reserved'].includes(row?.status)?row.status:'resident',linkLabel:localText(row?.linkLabel,fallback.linkLabel),note:localText(row?.note,fallback.note),intro:localText(row?.intro,fallback.intro),profile:normalizeCatProfile(row?.profile,fallback.profile),enabled:row?.enabled!==false};});
