...
export default function App() {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#f7f8f4] text-slate-950 selection:bg-sky-200">
    ...
    <main id="top" className="flex flex-col">
      ...
      <section id="cats" className="mx-auto max-w-6xl px-5 py-24 sm:px-8"><div className="mb-12 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><span className="text-sm font-extrabold uppercase tracking-[.2em] text-sky-600">{tr("Nasi gospodarze")}</span><h2 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{tr("Poznaj ekipę.")}</h2></div><p className="max-w-md text-slate-600">{tr("Każdy inny. Każdy u siebie. Pixel szuka domu, Luna i Mochi są stałymi gospodarzami.")}</p></div><div className="grid gap-5 md:grid-cols-3">{cats.map((cat, i) => <article key={cat.name} className={`group overflow-hidden rounded-[2rem] bg-white shadow-sm ${i === 1 ? 'md:translate-y-8' : ''}`}><div className="aspect-[4/5] overflow-hidden"><img src={cat.image} alt={tr("Kot {name}", {
                name: cat.name
      ...
      <section id="visit" className="mx-auto max-w-6xl px-5 pb-24 pt-10 sm:px-8"><div className="grid overflow-hidden rounded-[3rem] bg-white shadow-xl lg:grid-cols-[1fr_.9fr]"><div className="p-7 sm:p-12"><span className="text-sm font-extrabold uppercase tracking-[.2em] text-sky-600">{tr("Zanim wpadniesz")}</span><h2 className="mt-3 text-4xl font-black tracking-tight">{tr("Zaplanuj miękkie lądowanie.")}</h2><div className="mt-9 space-y-5"><InfoRow icon={Clock3} title={tr("Wt–Pt 11:00–20:00 · Sob–Nd 10:00–20:00")} text={tr("W poniedziałki lokal jest zamknięty — to dzień ciszy dla kotów.")} /><InfoRow icon={MapPin} title={tr("ul. Krakowska 32, 45-075 Opole")} text={tr("5 minut spacerem od opolskiego Rynku · adres koncepcyjny.")} /><InfoRow icon={CalendarDays} title={tr("Stoliki na 90 minut")} text={tr("W weekend rezerwacja zalecana. Dzieci od 8 lat, zawsze pod opieką dorosłego.")} /></div><div className="mt-9 flex flex-wrap gap-3"><button onClick={() => {
                setBookingEvent(null);
                setBookingOpen(true);
              }} className="flex items-center gap-2 rounded-full bg-sky-500 px-6 py-3.5 font-bold text-white transition hover:-translate-y-1 hover:bg-sky-600">{tr("Zarezerwuj stolik")}<CalendarDays size={18} /></button><button onClick={() => setChatOpen(true)} className="flex items-center gap-2 rounded-full border border-slate-300 px-6 py-3.5 font-bold hover:border-sky-400 hover:text-sky-600">{tr("Zapytaj asystenta")}<MessageCircle size={18} /></button><a href="https://www.openstreetmap.org/search?query=Krakowska%2032%20Opole" target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-full border border-slate-300 px-6 py-3.5 font-bold hover:border-sky-400 hover:text-sky-600">{tr("Pokaż mapę")}<MapPin size={18} /></a></div></div><div className="relative min-h-[380px]"><img src="https://images.unsplash.com/photo-1511081692775-05d0f180a065?q=85&w=1200&auto=format&fit=crop" alt={tr("Przytulne wnętrze kawiarni")} loading="lazy" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-slate-950/35 to-transparent" /></div></div></section>
      ...
      <section id="support" className="bg-slate-950 py-24 text-white"><div className="mx-auto max-w-6xl px-5 sm:px-8"><div className="grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-end"><div><span className="flex items-center gap-2 text-sm font-extrabold uppercase tracking-[.2em] text-sky-400"><HeartHandshake size={17} />{tr("Dla naszych kotów")}</span><h2 className="mt-4 text-4xl font-black leading-tight tracking-tight sm:text-5xl">{tr("Mały gest.")}<br />{tr("Dużo spokoju.")}</h2><p className="mt-5 max-w-md text-lg leading-relaxed text-slate-300">{tr("Opieka weterynaryjna, karma i spokojne kryjówki są ważniejsze niż kolejna zabawka. Wybierz sposób, w jaki chcesz dołożyć swoją cegiełkę.")}</p><button type="button" onClick={() => {
                ...
                setSupportOpen(true);
              }} className="rounded-3xl border border-white/10 bg-white/10 p-5 text-left transition hover:-translate-y-1 hover:bg-white/15"><Coffee className="text-amber-300" size={20} /><p className="mt-6 text-sm font-bold text-slate-300">{tr(title)}</p><p className="mt-1 text-3xl font-black">{amount}</p><p className="mt-2 text-xs leading-relaxed text-slate-400">{tr(text)}</p></button>)}</div></div><div className="mt-12 grid gap-5 rounded-[2.5rem] bg-white p-7 text-slate-950 sm:p-9 lg:grid-cols-[.8fr_1.2fr] lg:items-center"><div><p className="flex items-center gap-2 text-sm font-extrabold uppercase tracking-[.18em] text-sky-600"><ShoppingBasket size={17} />{tr("Lista potrzeb")}</p><h3 className="mt-3 text-3xl font-black">{tr("Co przyda się najbardziej?")}</h3><p className="mt-3 leading-relaxed text-slate-600">{tr("Rzeczy można przynieść po wcześniejszym kontakcie z obsługą. Najpierw sprawdzimy, czego aktualnie potrzebują koty.")}</p></div><ul className="grid gap-2 sm:grid-cols-2">{['mokra karma dobrej jakości', 'żwirek bentonitowy bez zapachu', 'polarowe koce i ręczniki', 'transportery w dobrym stanie', 'preparaty na pchły i kleszcze', 'środki do bezpiecznego sprzątania'].map(item => <li key={item} className="flex items-center gap-3 rounded-2xl bg-sky-50 px-4 py-3 text-sm font-bold text-slate-700"><Check size={16} className="shrink-0 text-emerald-500" />{tr(item)}</li>)}</ul></div></div></section>
      <section id="events" className="bg-sky-50 py-24"><div className="mx-auto max-w-6xl px-5 sm:px-8"><div className="mb-12 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><span className="text-sm font-extrabold uppercase tracking-[.2em] text-sky-600">{tr("W kalendarzu kociej kawiarni")}</span><h2 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{tr("Wydarzenia, na które chce się wracać.")}</h2></div><p className="max-w-md text-slate-600">{tr("Małe spotkania w spokojnym rytmie. Liczba miejsc jest ograniczona, dlatego warto zapisać się wcześniej.")}</p></div><div className="grid gap-4 md:grid-cols-2">{events.map(({
              icon: Icon,
              tag,
              title,
              text,
              meta
                  ...
                  setBookingOpen(true);
                }} className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-4 py-2.5 text-sm font-extrabold text-white transition hover:bg-sky-600">{tr("Zapisz się")}<ArrowRight size={15} /></button></div></article>)}</div></div></section>
