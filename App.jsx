...
const socialUrl = (value, network) => {
  ...
  const handle = text.replace(/^@/, '').replace(/^\/+|\/+$/g, '');
  return network === 'instagram' ? `https://www.instagram.com/${encodeURIComponent(handle)}` : `https://www.facebook.com/search/top?q=${encodeURIComponent(handle)}`;
};
const languageCopy = {
  pl: {
...
export default function App() {
  ...
  const publicEvents = siteContent.events.filter(item => item.enabled).map(item => {
    const original = baseEvents.find(event => event.id === item.id) || baseEvents[0];
    return { ...original, ...item, icon: original.icon, tag: contentText(item.tag), title: contentText(item.title), text: contentText(item.description), meta: contentText(item.date), spots: item.places };
  });
  ...
  const instagramUrl = socialUrl(cafe.instagram, 'instagram');
  const facebookUrl = socialUrl(cafe.facebook, 'facebook');
  ...
  return <div className="min-h-screen overflow-x-hidden bg-[#f7f8f4] text-slate-950 selection:bg-sky-200">
    ...
    <main id="top" className="flex flex-col">
      ...
      <section id="events" className="bg-sky-50 py-24"><div className="mx-auto max-w-6xl px-5 sm:px-8"><div className="mb-12 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><span className="text-sm font-extrabold uppercase tracking-[.2em] text-sky-600">{contentText(home.events.label)}</span><h2 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">{contentText(home.events.title)}</h2></div><p className="max-w-md text-slate-600">{contentText(home.events.text)}</p></div><div className="grid gap-4 md:grid-cols-2">{events.map(({
              ...
              meta,
              id
            }) => <article key={id} className="group rounded-[2rem] border border-sky-100 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg sm:p-7"><div className="flex items-start justify-between gap-4"><span className="grid h-12 w-12 place-items-center overflow-hidden rounded-2xl bg-sky-100 text-sky-600">{image ? <img src={image} alt="" className="h-full w-full object-cover"/> : <Icon size={22} />}</span><span className="rounded-full bg-amber-100 px-3 py-1.5 text-xs font-extrabold text-amber-800">{tr(tag)}</span></div><h3 className="mt-6 text-2xl font-black">{tr(title)}</h3><p className="mt-3 min-h-12 text-sm leading-relaxed text-slate-600">{tr(text)}</p><div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4"><span className="text-xs font-extrabold text-slate-500">{tr(meta)}</span><button type="button" onClick={() => {
                  setBookingEvent({
                    id,
                    title,
                    tag,
    ...
    <footer className="border-t border-slate-200 bg-white py-10"><div className="mx-auto flex max-w-6xl flex-col gap-6 px-5 sm:flex-row sm:items-center sm:justify-between sm:px-8"><div className="flex items-center gap-2 font-extrabold"><Cat className="text-sky-500" /> Niebieski Kot.</div><div className="text-sm text-slate-500"><p>{contentText(cafe.address)}</p><p className="mt-1"><a className="hover:text-sky-600" href={`mailto:${cafe.email}`}>{cafe.email}</a> · <a className="hover:text-sky-600" href={`tel:${cafe.phone.replace(/[^+\d]/g, '')}`}>{cafe.phone}</a></p><p className="mt-1 text-xs">{tr("Projekt koncepcyjny — adres, kontakt i bohaterowie są demonstracyjne.")}</p></div><div className="flex flex-wrap gap-4">{instagramUrl && <a href={instagramUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm font-bold hover:text-sky-600"><Instagram size={18}/>{cafe.instagram}</a>}{facebookUrl && <a href={facebookUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm font-bold hover:text-sky-600"><Globe2 size={18}/>{cafe.facebook}</a>}</div></div></footer>
